...
//     	dump($r);
    	$this->assign('uid',$uid);
    	$this->assign('r',$r);
    	$this->assign("umark","user_verify");
		$this->display("default/users/user_verify");
    }
...
//     	dump($c);die;
     ...
     * 检查用户是否认证
     */
 	public function check_users_vierfy(){
		$uid = session('WST_USER');
		$m = M('users_info');
		$info = $m->where('u_id='.$uid['userId'])->getfield('status');
// 		var_dump($info);die;
		if ($info==1) {
			echo "ok";die;
		}
